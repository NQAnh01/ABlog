...
type Config struct {
	...
	CommentBlocklistPath                                                                                                                              string
	CommentRateLimit                                                                                                                                  int
}

func Load() Config {
	return Config{
		Env: env("APP_ENV", "development"), Port: env("APP_PORT", "8080"), MongoURI: mongoURI(), MongoDatabase: env("MONGO_DATABASE", "lumina"), JWTSecret: env("JWT_SECRET", "development-only-change-me"), StorageType: env("STORAGE_TYPE", "local"), StoragePath: env("STORAGE_PATH", "uploads"), CloudinaryCloudName: os.Getenv("CLOUDINARY_CLOUD_NAME"), CloudinaryAPIKey: os.Getenv("CLOUDINARY_API_KEY"), CloudinaryAPISecret: os.Getenv("CLOUDINARY_API_SECRET"), ClientOrigin: env("CLIENT_ORIGIN", "http://localhost:5173"), SMTPHost: os.Getenv("SMTP_HOST"), SMTPPort: env("SMTP_PORT", "587"), SMTPUsername: os.Getenv("SMTP_USERNAME"), SMTPPassword: os.Getenv("SMTP_PASSWORD"), SMTPFrom: os.Getenv("SMTP_FROM"), AccessTTL: duration("JWT_ACCESS_EXPIRES", 15*time.Minute), RefreshTTL: duration("JWT_REFRESH_EXPIRES", 7*24*time.Hour), SeedAdminEmail: os.Getenv("SEED_ADMIN_EMAIL"), SeedAdminPassword: os.Getenv("SEED_ADMIN_PASSWORD"), CommentBlocklistPath: env("COMMENT_BLOCKLIST_PATH", "config/comment-blocklist.txt"), CommentRateLimit: integer("COMMENT_RATE_LIMIT", 6),
	}
}
...
func integer(k string, d int) int {
	if value, err := strconv.Atoi(os.Getenv(k)); err == nil && value > 0 {
		return value
	}
	return d
}
