...
export function LoginPage() {
  ...
  return <Layout><section className="auth-page"><div className="auth-card"><header><span className="auth-kicker">WELCOME TO LUMINA</span><h1>Welcome back</h1><p>Sign in to continue your stories and reading list.</p></header><form onSubmit={submit}>
    <Input label="Email address" name="email" type="email" autoComplete="email" placeholder="name@example.com" required/>
    <div className="password-row"><Input label="Password" name="password" type={show?'text':'password'} autoComplete="current-password" placeholder="••••••••" required/><button type="button" aria-label={show?'Hide password':'Show password'} onClick={()=>setShow(!show)}>{show?'Hide':'Show'}</button></div>
...
export function RegisterPage() {
  ...
  const [categories,setCategories]=useState<Category[]>([]);const [interests,setInterests]=useState<string[]>([])
  useEffect(()=>{api.categories().then(setCategories).catch(()=>setCategories([]))},[])
  if(user) return <Navigate to="/profile" replace/>
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);setError('');try{await register(String(f.get('name')),String(f.get('email')),String(f.get('password')),interests);navigate('/')}catch(err){setError(err instanceof Error?err.message:'Unable to register')}finally{setBusy(false)}}
  return <Layout><section className="register-page"><aside style={{backgroundImage:'url(https://images.unsplash.com/photo-1455390582262-044cdead277a?auto=format&fit=crop&w=1000&q=85)'}}><span className="auth-kicker">WRITE WITH INTENTION</span><blockquote>“The clearest way into the Universe is through a forest wilderness.”</blockquote><span>— JOHN MUIR</span></aside><div className="register-card"><header><span className="auth-kicker">CREATE YOUR ACCOUNT</span><h1>Join Lumina</h1><p>A thoughtful home for your writing and ideas.</p></header><form onSubmit={submit}><Input label="Full name" name="name" autoComplete="name" placeholder="Jane Doe" required/><Input label="Email address" name="email" type="email" autoComplete="email" placeholder="jane@example.com" required/><Input label="Password" name="password" type="password" autoComplete="new-password" minLength={8} placeholder="At least 8 characters" required/>{categories.length>0&&<fieldset className="registration-interests"><legend>What would you like to read?</legend><small>Choose topics to personalize Home and Explore.</small><div>{categories.map(category=><label key={category.id}><input type="checkbox" checked={interests.includes(category.id)} onChange={()=>setInterests(current=>current.includes(category.id)?current.filter(id=>id!==category.id):[...current,category.id])}/><span>{category.name}</span></label>)}</div></fieldset>}<label className="terms"><input type="checkbox" required/><span>I agree to Lumina's publishing guidelines and <Link to="/privacy">Privacy Policy</Link>.</span></label>{error&&<p className="form-error">{error}</p>}<Button disabled={busy}>{busy?'Creating account…':'Create account  →'}</Button></form><p className="auth-switch">Already have an account? <Link to="/login">Sign in</Link></p></div></section></Layout>
}
