...
export function RegisterPage() {
  ...
  return <Layout><section className="register-page"><aside style={{backgroundImage:'url(https://images.unsplash.com/photo-1455390582262-044cdead277a?auto=format&fit=crop&w=1000&q=85)'}}><span className="auth-kicker">WRITE WITH INTENTION</span><blockquote>“The clearest way into the Universe is through a forest wilderness.”</blockquote><span>— JOHN MUIR</span></aside><div className="register-card"><header><span className="auth-kicker">CREATE YOUR ACCOUNT</span><h1>Join Lumina</h1><p>A thoughtful home for your writing and ideas.</p></header><form onSubmit={submit}><Input label="Full name" name="name" autoComplete="name" placeholder="Jane Doe" required/><Input label="Email address" name="email" type="email" autoComplete="email" placeholder="jane@example.com" required/><Input label="Password" name="password" type="password" autoComplete="new-password" minLength={8} placeholder="At least 8 characters" required/><label className="terms"><input type="checkbox" required/><span>I agree to Lumina's publishing guidelines and <Link to="/privacy">Privacy Policy</Link>.</span></label>{error&&<p className="form-error">{error}</p>}<Button disabled={busy}>{busy?'Creating account…':'Create account  →'}</Button></form><p className="auth-switch">Already have an account? <Link to="/login">Sign in</Link></p></div></section></Layout>
}
