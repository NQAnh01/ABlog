...
export function ProfilePage() {
  ...
  return <Layout dark><section className="listing profile-stories container"><header className="profile-stories-hero"><div><span className="eyebrow">{user.role === 'admin' ? 'EDITORIAL LIBRARY' : 'YOUR WRITING'}</span><h1>{user.role === 'admin' ? 'All Stories' : 'My Stories'} <sup>{total}</sup></h1><p>Keep a story private for yourself or make it public for everyone.</p><div className="profile-summary" aria-label="Story summary"><span><i className="all"/>{total} total</span><span><i className="public"/>{publicCount} public</span><span><i className="private"/>{privateCount} private</span></div></div><div className="profile-heading-actions"><ViewToggle targetId="profile-stories-view" storageKey="profile-stories"/><Link className="button profile-create" to="/admin/posts/create"><span aria-hidden="true">＋</span> Create story</Link></div></header>
