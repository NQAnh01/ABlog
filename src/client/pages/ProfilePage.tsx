...
export function ProfilePage() {
  ...
  return <Layout dark><section className="listing profile-stories container"><header className="profile-stories-hero"><div><span className="eyebrow">{user.role === 'admin' ? 'EDITORIAL LIBRARY' : 'YOUR WRITING'}</span><h1>{user.role === 'admin' ? 'All Stories' : 'My Stories'} <sup>{total}</sup></h1><p>Keep a story private for yourself or make it public for everyone.</p><div className="profile-summary" aria-label="Story summary"><span><i className="all"/>{total} total</span><span><i className="public"/>{publicCount} public</span><span><i className="private"/>{privateCount} private</span></div></div><Link className="button profile-create" to="/admin/posts/create"><span aria-hidden="true">＋</span> Create story</Link></header>
    {error && <div className="admin-alert">{error}<button onClick={() => setError('')}>×</button></div>}{loading ? <Loading /> : posts.length === 0 ? <EmptyState title="Your first story starts here" text="Create a story and choose who can see it." /> : <div className="profile-story-grid">{posts.map((post,index) => <article style={{'--story-index':index} as React.CSSProperties} key={post.id} role="link" tabIndex={0} aria-label={`View ${post.title}`} onClick={() => navigate(`/stories/${post.id}/preview`)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); navigate(`/stories/${post.id}/preview`) } }}><div className="profile-story-cover">{post.thumbnail?.url ? <img src={post.thumbnail.url} alt="" /> : <div className="profile-cover-placeholder">L</div>}<span>Preview <b>↗</b></span></div><div className="profile-story-content"><button type="button" className={`profile-status ${post.status}`} disabled={changingStatus === post.id} title={`Change to ${post.status === 'public' ? 'private' : 'public'}`} onClick={event => { event.stopPropagation(); void toggleVisibility(post) }}>{changingStatus === post.id ? 'Saving…' : post.status}</button><h2>{post.title}</h2><p>{post.excerpt || 'No excerpt has been written yet.'}</p><footer><time>{post.updated_at ? `Updated ${new Intl.DateTimeFormat('en',{month:'short',day:'numeric'}).format(new Date(post.updated_at))}` : 'Ready to edit'}</time><Link to={`/admin/posts/${post.id}/edit`} onClick={event => event.stopPropagation()}>Edit story <span>→</span></Link></footer></div></article>)}</div>}
