...
export function ProfilePage() {
  ...
  useEffect(() => {
    ...
    api.myPosts('?limit=50').then(result => setPosts(result.items ?? [])).catch(err => setError(err instanceof Error ? err.message : 'Unable to load your stories')).finally(() => setLoading(false))
  }, [user])
  ...
  return <Layout dark><section className="listing profile-stories container"><header className="listing-head"><div><span className="eyebrow">{user.role === 'admin' ? 'EDITORIAL LIBRARY' : 'YOUR WRITING'}</span><h1>{user.role === 'admin' ? 'All Stories' : 'My Stories'} <sup>{posts.length}</sup></h1><p>Keep a story private for yourself or make it public for everyone.</p></div><div className="profile-actions"><Link className="editor-ghost" to="/profile/settings">Account settings</Link><Link className="button" to="/admin/posts/create">Write a new story</Link></div></header>
