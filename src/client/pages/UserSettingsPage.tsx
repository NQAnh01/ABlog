...
export function UserSettingsPage() {
  ...
  return <Layout><section className="account-settings container"><header><div><span className="eyebrow">ACCOUNT SETTINGS</span><h1>Your profile</h1><p>Keep your public information current and your account secure.</p></div><Link to="/profile">← Back to your stories</Link></header><div className="settings-layout"><aside><label className={`settings-avatar-upload${avatarBusy ? ' busy' : ''}`} title="Upload a new avatar"><span className="settings-avatar">{user.avatar ? <img src={user.avatar} alt={`${user.name}'s avatar`} /> : user.name?.[0]?.toUpperCase() ?? 'L'}</span><span className="settings-avatar-overlay">{avatarBusy ? 'Uploading…' : 'Change'}</span><input type="file" accept="image/jpeg,image/png,image/webp" disabled={avatarBusy} onChange={changeAvatar} /></label><small className="settings-avatar-help">JPEG, PNG or WebP · max 3 MB</small><strong>{user.name}</strong><span>{user.email}</span><nav><button className={tab === 'profile' ? 'active' : ''} onClick={() => switchTab('profile')}><i>○</i><span>Personal information<small>Name, email and phone</small></span></button><button className={tab === 'password' ? 'active' : ''} onClick={() => switchTab('password')}><i>◇</i><span>Password & security<small>Update your password</small></span></button></nav></aside><div className="settings-panel">{tab === 'profile' ? <><header><h2>Personal information</h2><p>This information identifies your account across Lumina.</p></header><form onSubmit={saveProfile}><label><span>Full name</span><input value={name} onChange={event => setName(event.target.value)} minLength={2} maxLength={80} required /></label><label><span>Email address</span><input value={user.email} readOnly disabled /><small>Email cannot be changed from this screen.</small></label><label><span>Phone number <em>Optional</em></span><input value={phone} onChange={event => setPhone(event.target.value)} inputMode="tel" placeholder="+84 901 234 567" maxLength={20} /><small>Digits, spaces, parentheses, + and - are accepted.</small></label>{error && <div className="settings-message error">{error}</div>}{success && <div className="settings-message success">{success}</div>}<footer><button className="button" disabled={busy}>{busy ? 'Saving…' : 'Save changes'}</button></footer></form></> : <><header><h2>Change password</h2><p>Use at least 8 characters. Your new password must differ from the current one.</p></header><form onSubmit={savePassword}>{([['current','Current password','current_password'],['next','New password','new_password'],['confirm','Confirm new password','confirm_password']] as const).map(([key,label,name]) => <label key={key}><span>{label}</span><div className="settings-password"><input name={name} type={show[key] ? 'text' : 'password'} minLength={8} maxLength={128} autoComplete={key === 'current' ? 'current-password' : 'new-password'} required /><button type="button" aria-label={`Show ${label.toLowerCase()}`} onClick={() => setShow(value => ({ ...value, [key]: !value[key] }))}>{show[key] ? 'Hide' : 'Show'}</button></div></label>)}{error && <div className="settings-message error">{error}</div>}{success && <div className="settings-message success">{success}</div>}<footer><button className="button" disabled={busy}>{busy ? 'Updating…' : 'Update password'}</button></footer></form></>}</div></div></section></Layout>
}
