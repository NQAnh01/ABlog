...
function AppearancePanel({
  ...
  function chooseFont(value: SiteFont) {
    setFont(value);
    previewFont(value);
  }
  async function save() {
    if (await onSave(font)) {
  ...
  return (
    <>
      <header>
        <h2>Appearance</h2>
        <p>Choose an accent color and the typography used across the site.</p>
      </header>
      <div className="theme-settings">
        ...
        >
          {accentThemes.map((theme) => (
            <button
              ...
              className={accent === theme.id ? "selected" : ""}
            ...
            >
              ...
              <span>
                ...
                <small>
                  {theme.id === "violet" ? "Lumina default" : "Preset theme"}
                </small>
              </span>
        ...
        <div className="font-options">
          ...
          {siteFonts.map((option) => (
            <button
              type="button"
              className={font === option.id ? "selected" : ""}
              onClick={() => chooseFont(option.id)}
              key={option.id}
            >
              <i>{option.name === "Modern" ? "Aa" : "Ag"}</i>
              <span>
                <b>{option.name}</b>
                <small>
                  {option.id === "editorial"
        ...
        <p>Changes are previewed immediately and applied after saving.</p>
        <footer>
          <button
            type="button"
            className="button"
            disabled={accent === committed && font === committedFont.current}
            onClick={() => void save()}
          >
            Save
          </button>
...
export function UserSettingsPage() {
  ...
  const [tab, setTab] = useState<
    "profile" | "appearance" | "password" | "data"
  >("profile");
  const [accent, setAccent] = useState<AccentTheme>(savedAccent);
  const [committedAccent, setCommittedAccent] =
    useState<AccentTheme>(savedAccent);
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      !(await confirmAction({
        ...
        message:
          "Your updated information will be shown on your account and public author profile.",
  ...
  async function savePassword(event: FormEvent<HTMLFormElement>) {
    ...
    const current = String(data.get("current_password") ?? ""),
      ...
      confirm = String(data.get("confirm_password") ?? "");
    setError("");
    if (next !== confirm) {
    ...
    if (
      !(await confirmAction({
        ...
        message:
          "Your password will be updated and other signed-in sessions may be closed.",
  ...
  async function handleExportData() {
    ...
    try {
      ...
      toast("Your personal data archive was downloaded successfully.");
    } catch (err) {
      setExportError(
        err instanceof Error ? err.message : "Unable to export data",
      );
    } finally {
      setExporting(false);
    }
  }
  function switchTab(value: "profile" | "appearance" | "password" | "data") {
    setTab(value);
    setError("");
    setExportError("");
  }
  ...
  async function saveAppearance(font: SiteFont) {
    if (
      !(await confirmAction({
        title: "Save appearance?",
        message: "This color and typography will be used across the site.",
        confirmLabel: "Save",
      }))
    )
      return false;
    ...
    committedAccentRef.current = accent;
    setCommittedAccent(accent);
    toast("Appearance updated successfully.");
    return true;
  }
  function addCustomLink() {
    setSocials((value) => ({
      ...value,
      links: [...(value.links ?? []), { name: "", url: "" }],
    }));
  }
  function updateCustomLink(index: number, key: "name" | "url", next: string) {
    setSocials((value) => ({
      ...value,
      links: (value.links ?? []).map((link, current) =>
        current === index ? { ...link, [key]: next } : link,
      ),
    }));
  }
  function removeCustomLink(index: number) {
    setSocials((value) => ({
      ...value,
      links: (value.links ?? []).filter((_, current) => current !== index),
    }));
  }
  return (
    <Layout>
      <section className="account-settings container">
        ...
        <div className="settings-layout">
          <aside>
            <label
              className={`settings-avatar-upload${avatarBusy ? " busy" : ""}`}
            ...
            <nav aria-label="Account settings">
              ...
              >
                ...
                <span>
                  Personal information<small>Public author details</small>
                </span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span>
                  Appearance<small>Color theme</small>
                </span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span>
                  Password & security<small>Update your password</small>
                </span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span>
                  Data & Privacy<small>GDPR portable archive</small>
                </span>
              </button>
            </nav>
            ...
            {tab === "profile" ? (
              <>
                <header>
                  ...
                  <p>Your bio and links appear on your public author page.</p>
                </header>
                <form onSubmit={saveProfile}>
                  ...
                  <label>
                    <span>
                      Author bio <em>{bio.length}/320</em>
                    </span>
                    <textarea
                      value={bio}
                      onChange={(event) => setBio(event.target.value)}
                      maxLength={320}
                      placeholder="Tell readers a little about your perspective and work."
                    />
                  </label>
                  {(
                    [
                  ...
                  <fieldset className="profile-interests">
                    <legend>Reading interests</legend>
                    <small>
                      Home and Explore use these categories to personalize your
                      stories.
                    </small>
                    <div>
                  ...
                  <section className="custom-links">
                    <header>
                      <div>
                        ...
                        <small>
                          Add up to 8 named links to your author page.
                        </small>
                      </div>
                      <button
                        type="button"
                        disabled={(socials.links?.length ?? 0) >= 8}
                    ...
                    {socials.links?.map((link, index) => (
                      <div className="custom-link-row" key={index}>
                        ...
                        <button
                          type="button"
                          aria-label={`Remove ${link.name || "custom link"}`}
              ...
              <>
                <header>
                  ...
                  <p>
                    Use at least 8 characters. Your new password must differ
              ...
              <>
                <header>
                  ...
                  <p>
                    ...
                    according to data portability standards.
                  </p>
                </header>
                <div className="gdpr-box">
                  <div className="gdpr-card">
                    <div className="gdpr-info">
                      <h3>Portable Data Archive (.json)</h3>
                      <p>
                        Your download includes a consolidated JSON export of:
                      </p>
                      <ul className="gdpr-list">
                        <li>
                          ✓ Your profile credentials, contact info and public
                          author bio
                        </li>
                        <li>
                          ✓ All published and private stories, revisions and
                          drafts
                        </li>
                        <li>
                          ✓ Your authored comments, discussion threads and
                          replies
                        </li>
                        <li>
                          ✓ Bookmarks, reading lists and productivity targets
                        </li>
                      </ul>
                    </div>
                  </div>
