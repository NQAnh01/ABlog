...
import { ArrowLeft, Palette, Plus, ShieldCheck, Trash2, UserRound } from 'lucide-react'
import { accentStorageKey, accentThemes, previewAccent, savedAccent, type AccentTheme } from '../theme'
import { confirmAction } from '../components/ConfirmModal'

function AppearancePanel({accent,committed,onSelect,onSave}:{accent:AccentTheme;committed:AccentTheme;onSelect:(value:AccentTheme)=>void;onSave:()=>void}) {
  return <><header><h2>Appearance</h2><p>Choose an accent color. Light and dark mode will use matching shades.</p></header><div className="theme-settings"><div className="theme-options" role="radiogroup" aria-label="Accent color">{accentThemes.map(theme=><button type="button" role="radio" aria-checked={accent===theme.id} className={accent===theme.id?'selected':''} onClick={()=>onSelect(theme.id)} key={theme.id}><i style={{background:theme.color}}/><span><strong>{theme.name}</strong><small>{theme.id==='violet'?'Lumina default':'Preset theme'}</small></span><b aria-hidden="true">✓</b></button>)}</div><p>Changes are previewed immediately. Leave without saving to restore your previous color.</p><footer><button type="button" className="button" disabled={accent===committed} onClick={onSave}>Save theme</button></footer></div></>
}
...
export function UserSettingsPage() {
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();if(!await confirmAction({title:'Save profile changes?',message:'Your updated information will be shown on your account and public author profile.',confirmLabel:'Save changes'}))return; setBusy(true); setError('')
  ...
  async function savePassword(event: FormEvent<HTMLFormElement>) {
    ...
    if(!await confirmAction({title:'Change your password?',message:'Your password will be updated and other signed-in sessions may be closed.',confirmLabel:'Change password',danger:true}))return
    setBusy(true)
  ...
  async function saveAppearance(){if(!await confirmAction({title:'Save this color theme?',message:'This accent color will be used with both light and dark mode.',confirmLabel:'Save theme'}))return;localStorage.setItem(accentStorageKey,accent);committedAccentRef.current=accent;setCommittedAccent(accent);toast('Theme updated successfully.')}
