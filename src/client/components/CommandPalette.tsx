...
type PaletteItem = { id: string; group: 'Actions' | 'Stories' | 'Categories' | 'Tags'; title: string; detail?: string; icon: string; run: () => void }
...
export function CommandPalette({ dark, onToggleTheme }: { dark: boolean; onToggleTheme: () => void }) {
  ...
  const items = useMemo(() => {
    const actions: PaletteItem[] = [
      ...(user ? [{ id:'new',group:'Actions' as const,title:'Create a new story',detail:'Open the editor',icon:'＋',run:closeAnd(()=>navigate('/admin/posts/create')) }] : []),
      ...(user?.role==='admin' ? [{ id:'dashboard',group:'Actions' as const,title:'Open Dashboard',detail:'Editorial overview',icon:'⌁',run:closeAnd(()=>navigate('/admin/dashboard')) }] : []),
      ...(user ? [{ id:'saved',group:'Actions' as const,title:'Saved stories',detail:'Your reading list',icon:'◇',run:closeAnd(()=>navigate('/saved')) }] : []),
      ...(user ? [{ id:'series',group:'Actions' as const,title:'Manage series',detail:'Curate a reading journey',icon:'▤',run:closeAnd(()=>navigate('/admin/series')) }] : []),
      { id:'theme',group:'Actions' as const,title:`Switch to ${dark?'light':'dark'} theme`,detail:'Change appearance',icon:dark?'☀':'◐',run:closeAnd(onToggleTheme) },
      ...(!user ? [{ id:'login',group:'Actions' as const,title:'Sign in',detail:'Access writing tools',icon:'→',run:closeAnd(()=>navigate('/login')) }] : []),
    ]
    if (!debounced) return actions
    const mergedPosts = [...new Map([...serverPosts,...posts].map(post=>[post.id,post])).values()]
    const storyItems = mergedPosts.map(post => ({ post, score:Math.max(fuzzyScore(post.title,debounced),fuzzyScore(post.author?.name??'',debounced)) })).filter(item=>item.score>0).sort((a,b)=>b.score-a.score).slice(0,8).map(({post})=>({id:`post-${post.id}`,group:'Stories' as const,title:post.title,detail:`By ${post.author?.name??'Lumina'}`,icon:'◫',run:closeAnd(()=>navigate(`/blog/${post.slug}`))}))
    const categoryItems = categories.map(category=>({category,score:fuzzyScore(category.name,debounced)})).filter(item=>item.score>0).sort((a,b)=>b.score-a.score).slice(0,5).map(({category})=>({id:`category-${category.id}`,group:'Categories' as const,title:category.name,detail:'Category',icon:'▦',run:closeAnd(()=>navigate(`/categories/${category.slug}`))}))
    const tagItems = tags.map(tag=>({tag,score:fuzzyScore(tag.name,debounced)})).filter(item=>item.score>0).sort((a,b)=>b.score-a.score).slice(0,5).map(({tag})=>({id:`tag-${tag.id}`,group:'Tags' as const,title:`#${tag.name}`,detail:'Tag',icon:'#',run:closeAnd(()=>navigate(`/tags/${tag.slug}`))}))
  ...
  return <div className="command-palette-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false)}}><section className="command-palette" role="dialog" aria-modal="true" aria-label="Command palette">
    <header><span aria-hidden="true">⌕</span><input ref={inputRef} role="combobox" aria-controls="command-results" aria-expanded="true" aria-activedescendant={items[selected]?.id} value={query} onChange={event=>setQuery(event.target.value)} onKeyDown={onKeyDown} placeholder="Search stories, authors, categories, or tags…"/><kbd>Esc</kbd></header>
    <div className="command-results" id="command-results" role="listbox" aria-busy={loading}>{loading&&<span className="command-loading"/>}{items.length?groups.map(group=>{const grouped=items.filter(item=>item.group===group);if(!grouped.length)return null;return <section className="command-group" key={group}><h2>{group}</h2>{grouped.map(item=>{const index=items.indexOf(item);return <button id={item.id} type="button" role="option" aria-selected={index===selected} className={index===selected?'selected':''} key={item.id} onMouseEnter={()=>setSelected(index)} onClick={item.run}><i>{item.icon}</i><span><strong>{item.title}</strong>{item.detail&&<small>{item.detail}</small>}</span><b>↵</b></button>})}</section>}):<div className="command-empty"><span>⌕</span><strong>No results found</strong><small>Try another title, author, category, or tag.</small></div>}</div>
