...
export function Header({ dark, onToggleTheme }: { dark: boolean; onToggleTheme: () => void }) {
  ...
  <div className={`mobile-nav-layer${mobileOpen?' open':''}`} aria-hidden={!mobileOpen} onMouseDown={event=>{if(event.target===event.currentTarget)setMobileOpen(false)}}><aside id="mobile-navigation" className="mobile-nav-drawer" aria-label="Mobile navigation"><header><div>{user?<><span className="avatar">{user.avatar?<img src={user.avatar} alt=""/>:user.name?.[0]?.toUpperCase()??'L'}</span><span><strong>{user.name}</strong><small>{user.email}</small></span></>:<><span className="logo-mark">L</span><span><strong>Explore Lumina</strong><small>Stories worth your time</small></span></>}</div><button type="button" aria-label="Close navigation" onClick={()=>setMobileOpen(false)}>×</button></header><button className="mobile-nav-search" type="button" onClick={openSearch}><span>⌕</span><strong>Search Lumina</strong><kbd>⌘K</kbd></button><nav><NavLink to="/" end><span>⌂</span>Home</NavLink><NavLink to="/blog"><span>◫</span>Explore</NavLink><NavLink to="/profile"><span>✎</span>My Stories</NavLink>{user?.role==='admin'&&<NavLink to="/admin/dashboard"><span>▦</span>Dashboard</NavLink>}{user&&<NavLink to="/saved"><span>◇</span>Reading List</NavLink>}{!user&&<NavLink to="/register"><span>＋</span>Sign Up</NavLink>}</nav><footer>{user&&<Link className="mobile-new-post" to="/admin/posts/create">＋ Create new story</Link>}<button type="button" onClick={onToggleTheme}><span>{dark?'☀':'◐'}</span>Switch to {dark?'light':'dark'} theme</button>{user?<><Link to="/profile/settings"><span>⚙</span>Account settings</Link><button type="button" onClick={()=>void signOut()}><span>→</span>Sign out</button></>:<Link to="/login"><span>→</span>Sign in</Link>}</footer></aside></div></>
}

export function Footer() { const{canInstall,install}=usePWA();return <footer className="site-footer"><div className="container footer-inner"><div><Logo/><p>© 2026 Lumina Publishing Group.</p></div><nav><Link to="/offline">Offline reading</Link>{canInstall&&<button className="pwa-install" type="button" onClick={()=>void install()}>＋ Install app</button>}<Link to="/about">About</Link><Link to="/privacy">Privacy</Link><Link to="/socials">Socials</Link></nav></div></footer> }
...
export function Layout({ children, dark: darkDefault = false }: { children: ReactNode; dark?: boolean }) {
  ...
  return <div className={dark ? 'theme-dark app-shell' : 'theme-light app-shell'}><Header dark={dark} onToggleTheme={toggleTheme}/><main>{children}</main><Footer/><ScrollFeedback/><CommandPalette dark={dark} onToggleTheme={toggleTheme}/></div>
}
