...
async function cacheArticle(article){const cache=await caches.open(ARTICLES);const apiRequest=new Request(article.apiUrl,{credentials:'omit'});let contentReady=Boolean(await cache.match(apiRequest));try{const response=await fetch(apiRequest);if(response.ok){await cache.put(apiRequest,response.clone());contentReady=true}}catch{/* Keep the current offline copy when refresh fails. */}if(!contentReady)throw new Error('Article content could not be cached');if(article.imageUrl){try{const image=await fetch(article.imageUrl,{mode:'cors'});if(image.ok)await cache.put(article.imageUrl,image.clone())}catch{/* The article text remains useful when a remote image disallows caching. */}}let values=(await readMeta()).filter(item=>item.id!==article.id);values.unshift({...article,cachedAt:Date.now()});for(const removed of values.slice(MAX_ARTICLES)){await cache.delete(removed.apiUrl);if(removed.imageUrl)await cache.delete(removed.imageUrl)}await writeMeta(values.slice(0,MAX_ARTICLES))}
